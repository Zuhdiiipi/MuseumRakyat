                ...
                <div class="bg-black d-flex align-items-center justify-content-center position-relative"
                    ...
                    @if ($artifact->category === 'BENDA')
                        ...
                        @if ($artifact->ai_status === 'SUCCESS' && $artifact->model_path)
                            {{-- Logika Cek URL: Apakah dari Tripo (HTTP) atau Lokal? --}}
                            @php
                                $modelSrc = '';
                                if (Str::startsWith($artifact->model_path, 'http')) {
                                    // Jika dari Tripo, pakai Proxy agar aman CORS
                                    $modelSrc = route('artifacts.proxy') . '?url=' . urlencode($artifact->model_path);
                                } else {
                                    // Jika file lokal (hasil download manual), pakai asset()
                                    $modelSrc = asset('storage/' . $artifact->model_path);
                                }
                            @endphp

                            <model-viewer src="{{ $modelSrc }}" poster="{{ asset('storage/' . $artifact->image_path) }}"
                                alt="Model 3D {{ $artifact->title }}" auto-rotate camera-controls ar shadow-intensity="1"
                        ...
                        @elseif($artifact->ai_status === 'PROCESSING' || $artifact->ai_status === 'PENDING')
                            <div class="text-center text-white" id="processingState">
                                ...
                                <p class="text-muted small">Mohon tunggu, AI sedang bekerja.<br>Halaman akan update
                                    otomatis.</p>
                            ...
                            {{-- Kontainer kosong untuk 3D nanti --}}
                            <div id="successState" class="d-none w-100 h-100"></div>

                            {{-- C. Jika Gagal atau Foto Biasa --}}
                        @else
                            <img src="{{ asset('storage/' . $artifact->image_path) }}" class="img-fluid"
                    ...
                    @else
                        <img src="{{ asset('storage/' . $artifact->image_path) }}" class="img-fluid w-100 object-fit-cover"
                            style="height: 500px;">
                    @endif
                </div>
            </div>
            ...
            <div class="card h-100 border-0" style="background: transparent;">
                <div class="card-body ps-lg-4">
                    ...
                    <div class="d-flex align-items-center mb-4 text-white-50">
                        ...
                        @if ($artifact->museum)
                            <div>
                                ...
                                <a href="{{ route('museums.show', $artifact->museum_id) }}"
                    ...
                    <div class="row g-2 mb-4 text-white-50 small">
                        ...
                        <div class="col-12 mt-2">
                            ...
                            @foreach ($artifact->tags as $tag)
                                <span
                                    class="badge bg-secondary border border-light text-white me-1">{{ $tag->name }}</span>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center">
                        <div class="me-auto text-white-50 small">
                        ...
                        @if (Auth::id() === $artifact->user_id || (Auth::user() && Auth::user()->isAdmin()))
                            <form action="{{ route('artifacts.destroy', $artifact->id) }}" method="POST"
...
@push('scripts')
    <script>
        // SCRIPT POLLING UNTUK CEK STATUS AI
        // Hanya dijalankan jika kategori BENDA dan status masih PROCESSING
        @if ($artifact->category === 'BENDA' && $artifact->ai_status === 'PROCESSING')

            const artifactId = {{ $artifact->id }};
            // Gunakan route helper Laravel agar URL lengkap & aman
            const checkUrl = "{{ route('artifacts.check_status', $artifact->id) }}";
            ...
            const poller = setInterval(() => {
                fetch(checkUrl)
                    ...
                    .then(response => {
                        ...
                        console.log("AI Status:", data.status); // Debug di Console

                        // KASUS 1: SUKSES
                        if (data.status === 'SUCCESS') {
                            clearInterval(poller); // Hentikan polling

                            // Sembunyikan loading, Tampilkan viewer
                            document.getElementById('processingState').classList.add('d-none');
                            const viewer = document.getElementById('successState');
                            viewer.classList.remove('d-none');

                            // Siapkan URL Proxy untuk Model 3D
                            const proxyUrl = "{{ route('artifacts.proxy') }}?url=" + encodeURIComponent(data
                                .model_url);

                            // Inject HTML Model Viewer
                            viewer.innerHTML = `
                        <model-viewer 
                            src="${proxyUrl}"
                    ...
                    `;
                            ...
                            // Refresh halaman otomatis setelah 2 detik agar data tersimpan rapi
                        ...
                        // KASUS 2: GAGAL
                            ...
                            // Tampilkan ERROR ASLI dari Controller agar bisa didebug
                            // (data.error dikirim dari catch block di Controller)
                            alert('Gagal Memproses 3D:\n' + (data.error || 'Unknown Error'));

                            // Reload halaman untuk update status di DB jadi FAILED
                            location.reload();
                        }
                    })
                    .catch(err => {
                        console.error("Polling Error:", err);
                        // Jangan stop poller dulu jika hanya error koneksi sesaat
                    });
